...
"""
MPH_model_skeleton_new.py
Mirage Image Correction Model Skeleton for Thesis Project

FIX: Modified ConvBlock forward pass structure for better stability against cuDNN errors.
"""
...
# ---------------------------------------------------------------
# Helper: Basic Convolutional Block (MODIFIED FOR STABILITY)
# ---------------------------------------------------------------
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        # Explicit forward pass is sometimes required to stabilize cuDNN.
        x = self.conv(x)
        x = self.bn(x)
        if self.act:
            x = self.relu(x)
