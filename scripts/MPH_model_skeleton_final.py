"""
MPH_model_skeleton_final.py
Mirage Image Correction Model Skeleton for Thesis Project

CRITICAL FIX: Implements FauxCUDNNConv2d to bypass persistent cuDNN internal errors 
in the TiltEstimator/ConvBlock.
"""
...
# ---------------------------------------------------------------
# CRITICAL FIX: Custom Conv2d to bypass CUDNN_STATUS_INTERNAL_ERROR
# ---------------------------------------------------------------
class FauxCUDNNConv2d(nn.Conv2d):
    """A wrapper for nn.Conv2d that forces CUDNN_ENABLED=False during forward.
    This resolves persistent CUDNN_STATUS_INTERNAL_ERROR seen in early convolutions.
    """
    def forward(self, input):
        # Temporarily disable cuDNN for this call
...
# ---------------------------------------------------------------
# Helper: Basic Convolutional Block (USING THE FIXED CONV LAYER)
# ---------------------------------------------------------------
class ConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, k=3, s=1, p=1, act=True):
        super().__init__()
        # CRITICAL: Use the specialized convolution layer
        self.conv = FauxCUDNNConv2d(in_ch, out_ch, k, s, p) 
...
class AlignmentModule(nn.Module):
    def __init__(self, feat_ch=64):
        super().__init__()
        self.query = FauxCUDNNConv2d(feat_ch, feat_ch // 8, 1) # Using FauxCUDNNConv2d here too
        self.key = FauxCUDNNConv2d(feat_ch, feat_ch // 8, 1)   # Using FauxCUDNNConv2d here too
        self.value = FauxCUDNNConv2d(feat_ch, feat_ch, 1)      # Using FauxCUDNNConv2d here too
...
class SimpleSSMBlock(nn.Module):
    def __init__(self, feat_ch=64):
        ...
        self.conv_gate = FauxCUDNNConv2d(feat_ch, feat_ch, 1) # Using FauxCUDNNConv2d here too
...
class Decoder(nn.Module):
    def __init__(self, in_ch=64, out_ch=3):
        ...
        self.decode = nn.Sequential(
            ...
            FauxCUDNNConv2d(in_ch // 4, out_ch, 3, 1, 1) # Using FauxCUDNNConv2d for final output layer
        )
