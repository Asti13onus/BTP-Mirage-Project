# (Paste the full code below, then Ctrl+O, Enter, Ctrl+X)
#!/usr/bin/env python3
"""
MPH_training_script5_fixed5.py

Final Integrated training script for Mirage Correction (MPH).
Includes all stability fixes and the necessary --cpu argument definition.
"""
import os
import argparse
import random
...
# Import your model skeleton (Assumed to be a stable version like final.py)
...
def parse_args():
    ...
    p.add_argument('--cpu', action='store_true', help='Force execution on CPU') # <--- FIXED
    return p.parse_args()
...
def main():
    ...
    device = 'cuda' if torch.cuda.is_available() and not cfg.cpu else 'cpu' # <--- USE cfg.cpu
    print("Using device:", device)

    # CRITICAL FIX 1: Enable cuDNN benchmarking for potential GPU use
    torch.backends.cudnn.benchmark = True 
    
    # CRITICAL FIX 2: Force deterministic convolutions (only matters if CUDA is active)
