"""
MPH_training_script5.py

Final Training script for the Mirage Correction Model (MPH) using actual .npz data.
This version incorporates all necessary fixes for a successful end-to-end run.
...
# Import all dependencies needed by the script
import numpy # Added for robustness in dataset worker process
...
class SyntheticTurbulenceDataset(Dataset):
    ...
    def __init__(self, root, split='train', T=5, crop_size=128, n_samples=2000):
        ...
        if split == 'train':
            ...
            # Logic adjusted for small smoke test (n_train=20, n_val=5)
            if n_samples < 100: 
                start_idx = 20 # Skip the first 20 used for train smoke test
                end_idx = 25   # Take the next 5 files
            else:
                 # Default split logic (assuming n_train default is 2000)
                 start_idx = 2000 
    ...
    def __getitem__(self, idx):
        ...
        # FIX: Use full name 'numpy.load' to resolve NameError in worker process
        data = numpy.load(self.file_paths[idx]) 
        
        # sequence: (T, H, W, C) float32 [0, 1]
        sequence = data['seq'].astype(numpy.float32)
        ...
        # FIX: Removed 'tilt_flow': None to resolve the DataLoader TypeError
        return {'frames': input_seq, 'gt': gt_frame, 'meta': {}}
    ...
    def train(self):
        ...
        for epoch in range(cfg.epochs):
            ...
            for batch_idx, sample in enumerate(self.train_loader):
                ...
                # We skip tilt_flow as it's not provided by the current dataset
                ...
                with autocast(enabled=not cfg.disable_amp):
                    ...
                    # loss_tilt is 0 since we don't have tilt_flow ground truth
            ...
            # --- FIX APPLIED HERE ---
            # Correctly use .state_dict() for saving the optimizer state
            ckpt = {
                'model': self.model.state_dict(),
                'opt': self.opt.state_dict(), # <--- CORRECTED
                'epoch': epoch,
                'config': vars(cfg),
            }
            # --- END FIX ---
