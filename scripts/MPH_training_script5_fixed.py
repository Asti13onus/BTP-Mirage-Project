#!/usr/bin/env python3
"""
MPH_training_script5_final.py

Integrated training script for Mirage Correction (MPH).
Features:
 - robust NPZ dataset loader with consistent framewise augmentation
 - AMP support (GradScaler) and AMP-safe gradient clipping
 - AdamW with weight decay
 - ReduceLROnPlateau scheduler
 - checkpointing: checkpoint_last.pth, checkpoint_best.pth (saves model/optimizer/scheduler/scaler)
 - resume-from-best/last using --resume_from
 - EarlyStopping
 - optional encoder freeze for first N epochs
 - simple differentiable in-loop simulator (stable)
 - logging to TensorBoard

Usage examples shown below this file.
"""
import os
import argparse
import random
...
class SyntheticTurbulenceDataset(Dataset):
    ...
    Each .npz should contain 'seq' shaped (T_full, H, W, C) float32 in [0,1].
    Applies consistent spatial augmentations across the T_train window.
    """
    def __init__(self, root: str, file_list: Optional[List[str]] = None,
        ...
        # color jitter transform instance (we will call it consistently per-window)
    ...
    def _apply_framewise_transforms(self, window: np.ndarray) -> np.ndarray:
        """
        window: (T, H, W, C) float32 [0,1]
        returns transformed window: (T, crop, crop, C) float32 [0,1]
        """
    ...
    def __getitem__(self, idx):
        ...
        seq = data[key].astype(np.float32)  # (T_full, H, W, C)
...
def simple_simulate(restored: torch.Tensor, lpd_mu: Optional[torch.Tensor] = None, noise_scale: float = 0.01):
    """
    Small, stable differentiable simulator:
     - depthwise 3x3 gaussian blur
     - low-frequency amplitude scintillation from blurred luminance
     - additive gaussian noise scaled by lpd_mu strength (if provided)
    """
    ...
    weight = kernel2d.repeat(C, 1, 1, 1)  # depthwise
    ...
    amp = 0.98 + 0.06 * amp  # in [0.98, 1.04]
...
def train_one_epoch(model, dataloader, optimizer, scaler, device, epoch, cfg):
    ...
    for batch in dataloader:
        ...
        with autocast(enabled=cfg.use_amp):
            restored, mu, logvar = model(frames)  # assume model returns restored (B,C,H,W), mu (B,z), logvar (B,z) or None
...
def freeze_encoder(model):
    # heuristic: freeze parameters that contain common encoder substrings
...
def main():
    ...
    start_epoch = 0
    best_val = float('inf')
    earlystop = EarlyStopping(patience=12, min_delta=1e-4)
    ...
    if cfg.resume_from:
        try:
            ...
            print(f"Resumed from {cfg.resume_from} at epoch {start_epoch}, best_val={best_val}")
