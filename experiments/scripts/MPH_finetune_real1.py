"""
MPH_finetune_real_final.py

Final, fixed Fine-tuning script for the Mirage Correction Model (MPH) using Real Data 
(BVI, OTIS, etc.) via self-supervised/unsupervised losses.

FIXED: Resolves NameError for 'seed_everything' and ensures correct checkpoint saving.
...
# ----------------------------- UTILITY FUNCTIONS (FIXED LOCATION) ----------------------#

def seed_everything(seed=42):
    # This must be defined globally before the Trainer class uses it.
    random.seed(seed)
...
class RealMirageDataset(Dataset):
    ...
    def __init__(self, root, split='train', T=5, crop_size=128, n_samples=1000):
        ...
        # --- Recursively find all PNG files in the real data subfolders ---
        self.all_frame_paths = []
        for dirpath, _, filenames in os.walk(self.root):
            for f in sorted(filenames):
                # Filter out potential single GT images, ensuring we only grab sequence frames
        ...
        # We need a list of starting frames (clip_starts) to extract T-frame sequences.
        if split == 'train':
            rng = random.Random(42)
            self.sequence_start_indices = rng.sample(range(total_frames - T), min(n_samples, total_frames - T))
        elif split == 'val':
            rng = random.Random(43) # Use a different seed for validation
            # Start validation sampling after the training block, or randomly for now
            self.sequence_start_indices = rng.sample(range(total_frames - T), min(n_samples, total_frames - T))
        else:
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.T):
            ...
            # --- TEMPORARY: Load dummy data if image libraries are missing ---
            C = 3
            H = W = self.crop_size
            frame_arr = np.random.rand(H, W, C).astype(np.float32) # Using np from global import
            # --- END TEMPORARY ---
            
            # Convert (H, W, C) to PyTorch (C, H, W)
...
class TemporalConsistencyLoss(nn.Module):
    """
    Self-supervised loss penalizing flicker between adjacent restored frames.
    """
...
class SpatialSmoothnessLoss(nn.Module):
    """Total Variation (TV) Loss to encourage smooth (non-noisy) output."""
    def __init__(self):
        super().__init__()
        self.l1 = nn.L1Loss()
...
class FineTuneTrainer:
    def __init__(self, config):
        # FIX: seed_everything is now defined and accessible globally
        ...
        if config.pretrained:
            ...
            # The synthetic model checkpoint structure uses the 'model' key
        ...
        for epoch in range(cfg.epochs):
            ...
            for batch_idx, sample in enumerate(self.train_loader):
                ...
                with autocast(enabled=not cfg.disable_amp):
                    # Forward Pass
                    restored, _, _ = self.model(frames) # restored is (B, C, H, W)
                    ...
                    total_loss = self.temp_weight * loss_temp + self.smooth_weight * loss_smooth
...
def get_argparser():
    ...
    p.add_argument('--data', type=str, required=True, help='Path to real data (BVI/OTIS)')
...
if __name__ == '__main__':
    ...
    # We use a simple wrapper class to initialize and run the trainer
